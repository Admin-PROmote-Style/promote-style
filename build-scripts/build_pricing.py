...
.glance-item .num{display:block;font-size:18px;font-weight:800;color:var(--gold);font-family:var(--font-head);overflow-wrap:break-word;hyphens:auto}
...
/* Spanish text runs longer than English (site sets <html lang="es"> when
   the ESP toggle is on -- see lang_script() in site_common.py). min-width:0
   above stops the grid itself from overflowing the page, but a single long
   unbroken word (e.g. "Impulsados") in the bold .num line still visually
   overflowed into the next column since it had no wrap/hyphenation hint of
   its own -- fixed above. Give both lines a little extra room in ESP too. */
html[lang="es"] .glance-item .num{font-size:15px}
